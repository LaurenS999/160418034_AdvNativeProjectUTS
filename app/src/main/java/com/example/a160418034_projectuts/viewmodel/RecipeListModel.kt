...
class RecipeListModel(application: Application): AndroidViewModel(application), CoroutineScope  {
    ...
    fun refresh(){
        ...
        launch {
            ...
            RecipeLD.value = db.recipeDao().selectAllTodo()
        }
    }
    ...
    fun clearTask(recipe: Recipe) {
        launch {
            val db = Room.databaseBuilder(
                    getApplication(),
                    RecipeDatabase::class.java, "newrecipedb").build()
            db.recipeDao().deleteTodo(recipe)
